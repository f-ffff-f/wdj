...
import WaveformVisualizer from '@/components/MainView/DJController/Waveform/WaveformVisualizer'
import FileUploader from '@/components/MainView/DJController/TrackLibrary/FileUploader'
    ...
    ({ playbackTime, audioBufferDuration }: { playbackTime: number; audioBufferDuration: number }) => {
        return (
            <Label className="min-w-[10ch] inline-block text-center text-xs">
                {formatTimeUI(playbackTime)} / -
                {Number.isFinite(audioBufferDuration) ? formatTimeUI(playbackTime - audioBufferDuration) : '∞'}
            </Label>
        )
    },
)
