...
const List = () => {
    ...
    const handleLoadToDeck = (deckId: number, url: string) => {
...
interface ITrackListItemProps {
    ...
    url: string
    isFocused: boolean
    handleLoadToDeck: (deckId: number, url: string) => void
...
const Item: React.FC<ITrackListItemProps> = ({ id, fileName, url, isFocused, handleLoadToDeck, handleClick }) => {
    return (
        ...
        >
            <Button onClick={() => handleLoadToDeck(1, url)}>load to deck 1</Button>
            <span className="flex-1 text-center px-4">
                ({id.slice(0, 4)}..) {fileName}
            </span>
            <Button onClick={() => handleLoadToDeck(2, url)}>load to deck 2</Button>
        </Card>
    )
}
